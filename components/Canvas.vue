      ...
      roll() {
        ...
        for (let i = 0; i < that.op.length; i++) {
          ...
          that.now[i] =
            ...
            (
              ((v(that.r) * 127 + 128) << 16) |
              ((v(that.r + u / 3) * 127 + 128) << 8) |
              (v(that.r + (u / 3) * 2) * 127 + 128)
            ).toString(16) +
            that.op[i];
        }
      },
...
export default {
  name: "Canvas",
  mounted() {
    evanyou.init(this.$refs.canvas);
    wave.init(this.$refs.canvas);
    this.doWithNightCheck();
  },
  data() {
    return {
      curIsNight: this.mustom$IsNight,
      curSkin: this.mustom$Skin
    }
  },
  computed: {
    dataWatched() {
      return {
        isNight: this.mustom$IsNight
      };
    }
  },
  watch: {
    dataWatched(data) {
      if (this.curIsNight !== data.isNight) {
        this.curIsNight = data.isNight;
        this.doWithNightCheck(data.isNight);
      }
    }
  },
  methods: {
    doWithNightCheck(isNight) {
      if (isNight || this.mustom$IsNight) {
        this.draw("evanyou");
      } else {
        this.draw("wave");
      }
    ...
    draw(opt) {
      ...
      } else if (opt.toLowerCase() === "evanyou") {
